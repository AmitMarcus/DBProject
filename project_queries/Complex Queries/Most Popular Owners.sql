SELECT Owner.id AS Owner_ID,Owner.name AS Owner_Name,OwnerAttTable.OwnerAttendings AS Owner_Events_Attendings,COUNT(Comment.id) AS Num_Of_Comments
FROM Owner,Event,Event_Owner,Comment,(SELECT Owner1.id AS OwnerID1, SUM(EventGuests1.attending_count) AS OwnerAttendings
													...
													WHERE Event1.id=EventOwner1.event_id and EventOwner1.owner_id=Owner1.id and Event1.id=EventGuests1.event_id
													GROUP BY Owner1.id
