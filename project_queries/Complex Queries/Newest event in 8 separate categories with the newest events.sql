SELECT Event.id AS Event_ID,Event.name AS Event_Name,Category.name AS Category_Name,Event_Time.start_time AS Event_Start_Time,Event_Desc_Search.description AS Event_Description
FROM Event,Category,Event_Time,Event_Desc_Search,(SELECT min(Event2.id) AS Newest_Event_ID
...
WHERE Event.id=EventIDS.Newest_Event_ID and Event.category_id=Category.id and Event.id=Event_Time.event_id and Event.id=Event_Desc_Search.event_id;
