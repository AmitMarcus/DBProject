SELECT City.name AS City_Name,Event.id AS Event_ID,Event.name AS Event_Name,Category.name AS Category_Name,Event_Desc_Search.description AS Event_Description,DATE(Event_Time.start_time) AS EventStartDate,DATE(Event_Time.end_time) as EventEndDate
FROM Event,Street,City,Category,Place,Event_Place,Event_Time,Event_Desc_Search,(SELECT City1.id AS CityId1 ,Count(Event1.id) As Num_Of_Events
